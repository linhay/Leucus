...
final class GhosttySurfaceView: NSView, Identifiable {
  ...
  override func doCommand(by selector: Selector) {
    ...
    if markedText.length == 0 {
      ...
      case #selector(deleteBackward(_:)):
        sendCommandSequence("\u{7F}")
        return
      case #selector(deleteForward(_:)):
        sendCommandSequence("\u{1B}[3~")
        return
      case #selector(moveLeft(_:)):
        sendCommandSequence("\u{1B}[D")
        return
      case #selector(moveRight(_:)):
        sendCommandSequence("\u{1B}[C")
        return
      case #selector(moveUp(_:)):
        sendCommandSequence("\u{1B}[A")
        return
      case #selector(moveDown(_:)):
        sendCommandSequence("\u{1B}[B")
        return
      case #selector(moveToBeginningOfLine(_:)):
        sendCommandSequence("\u{1B}[H")
        return
      case #selector(moveToEndOfLine(_:)):
        sendCommandSequence("\u{1B}[F")
        return
      default:
        break
      }
  ...
  private func sendCommandSequence(_ sequence: String) {
    insertText(sequence, replacementRange: NSRange(location: 0, length: 0))
  }
